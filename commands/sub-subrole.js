...
module.exports = {
    ...
    isim: "subrole", 
    alternatifler: ["sub-subscriberrole", "abonerolü"],
     kullanımı:"help",
...
const aset = new Discord.MessageEmbed()
    .setDescription(`<:hammer:897863047991947324> ・ The subscriber role already been set up.`)
    .setFooter(`Elenor - Subscriber System`)

const set = new Discord.MessageEmbed()
    .setDescription(`<:hammer:897863047991947324> ・ The subscriber role has been successfully set up.`)
    .setFooter(`Elenor - Subscriber System`)

const areset = new Discord.MessageEmbed()
    .setDescription(`<:hammer:897863047991947324> ・ The subscriber role is not already set.`)
    .setFooter(`Elenor - Subscriber System`)

const reset = new Discord.MessageEmbed()
    .setDescription(`<:hammer:897863047991947324> ・ The subscriber role has been successfully reset.`)
    .setFooter(`Elenor - Subscriber System`)
...
const zatenayarlı = new Discord.MessageEmbed()
    .setDescription(`<:hammer:897863047991947324> ・ Abone rolü zaten ayarlı.`)
    .setFooter(`Elenor - Abone Sistem`)

const ayarlandı = new Discord.MessageEmbed()
    .setDescription(`<:hammer:897863047991947324> ・ Abone rolü başarıyla ayarlandı.`)
    .setFooter(`Elenor - Abone Sistem`)

const zatensıfırlanmış = new Discord.MessageEmbed()
    .setDescription(`<:hammer:897863047991947324> ・ Abone rolü zaten ayarlı değil.`)
    .setFooter(`Elenor - Abone Sistem`)

const sıfırlandı = new Discord.MessageEmbed()
    .setDescription(`<:hammer:897863047991947324> ・ Abone rolü başarıyla sıfırlandı.`)
    .setFooter(`Elenor - Abone Sistem`)
...
if(en) {
  ...
  if (args[0] == 'set' || args[0] == 'Set') {
    if(db.has(`subrole.${message.guild.id}`)) return message.channel.send({embeds: [aset]})
    let bot = message.mentions.roles.first();
    if(!bot) return message.channel.send({embeds: [m]})
   
    db.set(`subrole.${message.guild.id}`, bot.id)
      message.channel.send({embeds: [set]})
  }
  if (args[0] == 'reset' || args[0] == 'Reset') {
        if(!db.has(`subrole.${message.guild.id}`)) return message.channel.send({embeds: [areset]})
    db.delete(`subrole.${message.guild.id}`)
...
if(tr) {
    ...
    if (args[0] == 'ayarla' || args[0] == 'Ayarla') {
        if(db.has(`subrole.${message.guild.id}`)) return message.channel.send({embeds: [zatenayarlı]})
      let bot = message.mentions.roles.first();
    ...
    if (args[0] == 'sıfırla' || args[0] == 'Sıfırla' ) {
          if(!db.has(`subrole.${message.guild.id}`)) return message.channel.send({embeds: [zatensıfırlanmış]})
      db.delete(`subrole.${message.guild.id}`)
